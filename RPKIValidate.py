...
from certvalidator._errors import pretty_message
from certvalidator._types import str_cls, type_name
...
def validate_ip_subnets(validation_context, path):
    """
    ...
    for x in path:
    #    print (x.subject.human_friendly)

        oldipx = ip_network("0.0.0.0/0")
        e = x['tbs_certificate']['extensions']
        for ex in e:
            if ex['extn_id'].native == 'id-pe-ipAddrBlocks':
                if  (ex['extn_value'].parse()[0][1].native) is not None:
                    ip = ex['extn_value'].parse()[0][1][0].contents
                    ipx = converttonetwork(ip)
                    ipx = ip_network(ipx)
                    if not  ipx.subnet_of(oldipx):
                        raise PathValidationError("Not a valid subnet")
                    oldipx = ipx
