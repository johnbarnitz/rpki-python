...
from py3779 import IPAddrBlocks, IPAddress, IPAddressRange
from collections import defaultdict


log = logging.getLogger(__name__)
def convertToRange(ip, version):
    if isinstance(ip, IPAddressRange):
        if version == 4:
            ipmin = IPv4Address(ip['min'].toMinIPAddress(version))
            ipmax = IPv4Address(ip['max'].toMaxIPAddress(version))
        elif version == 6:
            ipmin = IPv6Address(ip['min'].toMinIPAddress(version))
            ipmax = IPv6Address(ip['max'].toMaxIPAddress(version))
    elif isinstance(ip, IPAddress):
        if version == 4:
            tv = IPv4Network(ip.toIPAddress(version))
        elif version == 6:
            tv = IPv6Network(ip.toIPAddress(version))
        ipmin = tv.network_address
        ipmax = tv.broadcast_address
    return {'min': ipmin, 'max': ipmax}

def isChildNetwork( ip1, ip2):
    if ip1['min'] >= ip2['min'] and ip1['max'] <= ip2['max']:
        return True
    else:
        return False

def isChildNetwork_old(compareip, p, version):
#    log.info(f"isChildNetwork {compareip}, {p}, {version}")
    if (isinstance(compareip, IPAddress)) and isinstance(p, IPAddress):
        if version == 4:
            ipa = IPv4Network(compareip.toIPAddress(version))
            ipprev = IPv4Network(p.toIPAddress(version))
        elif version == 6:
            ipa = IPv6Network(compareip.toIPAddress(version))
            ipprev = IPv6Network(p.toIPAddress(version))
        if ipa.subnet_of(ipprev):
            return True
        else:
            return False
    elif (isinstance(compareip, IPAddress)) and isinstance(p, IPAddressRange):
        if version == 4:
            ipa = IPv4Network(compareip.toIPAddress(version))
            ipprevmin = IPv4Address(p['min'].toMinIPAddress(version))
            ipprevmax = IPv4Address(p['max'].toMaxIPAddress(version))
        elif version == 6:
            ipa = IPv6Network(compareip.toIPAddress(version))
            ipprevmin = IPv6Address(p['min'].toMinIPAddress(version))
            ipprevmax = IPv6Address(p['max'].toMaxIPAddress(version))
        if ipa.network_address >= ipprevmin and ipa.broadcast_address <= ipprevmax:
            return True
        else:
            return False
    elif (isinstance(compareip, IPAddressRange)) and isinstance(p, IPAddress):
        if version == 4:
            ipprev = IPv4Network(p.toIPAddress(version))
            ipamin = IPv4Address(compareip['min'].toMinIPAddress(version))
            ipamax = IPv4Address(compareip['max'].toMaxIPAddress(version))
        elif version == 6:
            ipprev = IPv6Network(p.toIPAddress(version))
            ipamin = IPv6Address(compareip['min'].toMinIPAddress(version))
            ipamax = IPv6Address(compareip['max'].toMaxIPAddress(version))
        if ipamin >= ipprev.network_address and ipamax <= ipprev.broadcast_address:
            return True
        else:
            return False
    elif (isinstance(compareip, IPAddressRange)) and isinstance(p, IPAddressRange):
        if version == 4:
            ipprevmin = IPv4Address(p['min'].toMinIPAddress(version))
            ipprevmax = IPv4Address(p['max'].toMaxIPAddress(version))
            ipamin = IPv4Address(compareip['min'].toMinIPAddress(version))
            ...
            ipamin = IPv6Address(compareip['min'].toMinIPAddress(version))
            ipamax = IPv6Address(compareip['max'].toMaxIPAddress(version))
        if ipamin >= ipprevmin and ipamax <= ipprevmax:
            return True
        else:
            return False
...
def convertaddrfam(af):
    return int.from_bytes(af.native, "big")

def get_id_pe_ipAddrBlocks(e):
    for ex in e:
        if ex['extn_id'].native == 'id-pe-ipAddrBlocks':
            return ex

def validate_ip_subnets(validation_context,cert, path):
    """
    Validate that the subnets in ipAddrBlocks are properly delegated, so that the signing
    certificate is responsible for that address space, and that address space is not
    ...
    pathlen = len(path)
    # I am going to Document this to figure out why it is slow

    # How many paths are we looking at ?

    log.debug(f"Starting validate_ip_subnets: Number of paths {pathlen}")
    ipstruct = []
    for certificate in path:

        # get the ipAddrBlocks extension
        e = certificate['tbs_certificate']['extensions']
        ex = get_id_pe_ipAddrBlocks(e)
        if ex is None:
            # if there is no extension, check if this is the first or last cert,
            # the missing extension is only allowed in middle certificates
            # = We don't seem to see this message, so probably not why it's slow
            if pathcount == 0 or pathcount == pathlen - 1:
                log.error (" ERROR: No IP Addresses found in first or last certificate")
                raise PathValidationError(f"No IP Addresses found in first or last certificate")
            else:
                log.info("INFO: No IP Addresses found in certificate, skipping")
            continue
        # Get all the blocks
        ipdict = {}
        # for each IP version (4/6)
        for ipblock in ex['extn_value'].parse(IPAddrBlocks):
            # If there is an IP Address, if not we just inherit the previous block
            # Check if it is ipv4
            if convertaddrfam(ipblock['addressFamily']) == 1:
                log.debug("Version IPv4")
                version = 4
            else:
                log.debug("Version IPv6")
                version = 6
            # if the choice exists
            if ipblock['ipAddressChoice'].chosen.native:
                # Loop through each address
                log.debug(f"Number of ipc {len(ipblock['ipAddressChoice'].chosen)}")
                iplist = []
                for ipc in ipblock['ipAddressChoice'].chosen:
                    iplist.append(ipc)
                ipdict[version] = iplist
                log.debug(f"List: {iplist}")
        ipstruct.append(ipdict)
        log.debug(f"Dict: {ipdict}")
        pathcout = pathcount + 1
    log.debug (f"Struct: {ipstruct}")
    log.debug("Leaving validate_ip_subnets")
#    with open(f"ipa/state.{uuid.uuid4().hex}.bin", "wb") as f: # "wb" because we want to write in binary mode
#        pickle.dump(ipstruct, f)
# Start other validation from CheckChildNetworks2
    ipstruct.reverse()
    success = True
    for ipver in [4,6]:
        k = []
        for v in ipstruct:
            if ipver in v:
                w = [ convertToRange(z.chosen, ipver) for z in v[ipver]]
                k.append(w)
        for p in k[0]:
            prev =[p]
            for s in range(len(k) - 1):
                ...
                for i in prev:
                    for t in k[s+1]:
                        if isChildNetwork(i, t):
                            log.debug(f"{i} is_subnet  {t}")
                            newprev.append(t)
                            break
                prev = newprev.copy()
                newprev.clear()
            if len(prev) > 0:
                log.debug(f"Validating {p} success")
            else:
                log.debug (f"Validating {p} failed")
                success = False
    if success:
        log.debug ("All validated")
    else:
        log.debug ("Failed validated")
        raise InvalidCertificateError(pretty_message(
            '''
            The X.509 certificate provided contains a network that is 
            not validated by intermediate certificates
            '''
        ))

# End other validation

def validate_ip_subnets_old(validation_context,cert, path):
    """
    Remove this, for reference only
    ...
    prev = [IPv4Network('0.0.0.0/0'), IPv6Network('::/0')]
    previous = defaultdict(list)

    pathcount = 0
    for x in path:
        newprevious = defaultdict(list)
        e = x['tbs_certificate']['extensions']
        for ex in e:
            if ex['extn_id'].native == 'id-pe-ipAddrBlocks':
                tmpprev = []
                allprev = []
                for blk in ex['extn_value'].parse(IPAddrBlocks):
                    # If there is an IP Address, if not we just inherit the previous block
                    if blk['ipAddressChoice'].chosen.native:
                        # Loop through the choices
                        for ipc in blk['ipAddressChoice'].chosen:
                            # There can be multiple subnets here
                            # Check if it is ipv4
                            if convertaddrfam(blk['addressFamily']) == 1:
                                # This is an IPAddress
                                if (isinstance(ipc.chosen, IPAddress)):
                                    ipa =  IPv4Network(ipc.chosen.toIPAddress(4))
                                # This is an IPAddress Range
                                elif (isinstance(ipc.chosen, IPAddressRange)):
                                    # TODO:  Not really sure what this is exactly, there are subnets
                                    # TODO: for min and max, but the max is a subnet of the min, so maybe
                                    # TODO: there is some kind of exclusion going on, but I'm not really
                                    # TODO: sure how to handle it, do they both get roas or is one included
                                    # TODO: some other roa . Looks like the min gets a roa, and the max
                                    # TODO: is ARIN owned address space.
                                    ipa = None
                                    print ("Address Range Min", ipc.chosen['min'].toIPAddress(4))
                                    print ("Address Range Max", ipc.chosen['max'].toIPAddress(4))
                                version = 4
                            else:
                                # This is an IPAddress v6
                                if (isinstance(ipc.chosen, IPAddress)):
                                    ipa =  IPv6Network(ipc.chosen.toIPAddress(6))
                                # This is an IPAddress v6 Range
                                elif (isinstance(ipc.chosen, IPAddressRange)):
                                    ipa = None
                                    print ("Address Range Min", ipc.chosen['min'].toIPAddress(6))
                                    print ("Address Range Max", ipc.chosen['max'].toIPAddress(6))
                                version = 6
                            if ipa:
                                allprev.append(ipa)
                                for p in prev:
                                    if p.version == version and ipa.subnet_of(p):
                                        tmpprev.append(ipa)
                            if pathcount == 0:
                                newprevious[version].append(ipc.chosen)
                                print(f"New Previous {newprevious[version]}")
                            else:
                                print(f"Searching for {ipc.chosen}")
                                for p in previous[version]:
                                    if isChildNetwork(ipc.chosen, p, version) or pathcount == 0:
                                        newprevious[version].append(ipc.chosen)
                                        print (f"New Previous {newprevious[version]}")
                                        break
                                else:
                                    print(f" Verror Network {ipc.chosen} not found in {previous[version]} count {pathcount}")

                    else:
                        # if there is no IP network, just accept the previous ones
                        tmpprev = prev
                        allprev = oldallprev
                prev = tmpprev
                oldallprev = allprev
    ...
    if (sorted([x for x in prev if x.version == 4 ] ) == sorted([x for x in allprev if x.version == 4 ] )
            and sorted([x for x in prev if x.version == 6 ] ) == sorted([x for x in allprev if x.version == 6 ] )):
        print ("all networks valid")
    else:
        print ("Exiting Validation", prev, allprev)
#        exit()
#        raise PathValidationError("Not a valid subnet", prev, allprev)
