...
def isChildNetwork(compareip, p, version):
        ...
        if ex is None:
            ...
            for blk in ex['extn_value'].parse(IPAddrBlocks):
                ...
                if blk['ipAddressChoice'].chosen.native:
                    ...
                    log.debug(f"Number of ipc {len(blk['ipAddressChoice'].chosen)}")
                    for ipc in blk['ipAddressChoice'].chosen:
                        # There can be multiple subnets here
                        # if it's the first entry, we'll accept the root address block
                        if pathcount == 0:
                            newprevious[version].append(ipc.chosen)
                        else:
                            # Try to find a subnet from the previous certificate that this certificate is a child of
 #                          log.debug(f"Find if  ischildnetwork {previous}")
                            for p in previous[version]:
#                              log.debug(f"Find if  ischildnetwork {ipc.chosen} in {p}")
                                if isChildNetwork(ipc.chosen, p, version):
#                                    log.debug("Is ischildnetwork")
                                    newprevious[version].append(ipc.chosen)
                                    log.debug(f"len newprevious {len(newprevious[version])} len previous {len(previous[version])}")
                                    break
                            else:
                                log.error(f" Verror Network {ipc.chosen} not found in {previous[version]} count {pathcount}")
                                raise PathValidationError(f"Not a valid subnet {ipc.chosen} not found in {previous[version]} count {pathcount}")
                    previous[version] = newprevious[version].copy()
                    newprevious[version].clear()
                else:
                    # if there is no IP network, just accept the previous ones
                    # Don't clear previous because you need it to carry over
                    pass
